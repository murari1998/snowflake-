...
-- gold dimanision table (descriptive data)
...
-- Gold Fact Table (Numeric / Metrics)
...
-- task -Load Dimension Gold
-- ALTER TASK projectdb.table_schema_dim.process_customer_scd2 SUSPEND;

CREATE OR REPLACE TASK projectdb.table_schema_dim.load_dim_customer_gold
WAREHOUSE = COMPUTE_WH
AFTER projectdb.table_schema_dim.process_customer_scd2
AS
INSERT OVERWRITE INTO projectdb.gold_schema.dim_customer_gold
SELECT
    customer_id,
    customer_name,
    email,
...
CREATE OR REPLACE TASK projectdb.table_schema_dim.load_fact_customer_gold
WAREHOUSE = COMPUTE_WH
AFTER projectdb.table_schema_dim.load_dim_customer_gold
AS
INSERT OVERWRITE INTO projectdb.gold_schema.fact_customer_metrics_gold
SELECT
    customer_id,
    customer_status AS credit_rating,
    CASE
        WHEN customer_status = 'A' THEN 100
        WHEN customer_status = 'B' THEN 80
        WHEN customer_status = 'C' THEN 60
        ELSE 40
    END,
    eff_start_dt
FROM projectdb.table_schema_dim.dim_customer_silver
...
SHOW TASKS IN DATABASE projectdb;

-- resume task
ALTER TASK projectdb.table_schema_dim.load_fact_customer_gold RESUME;
ALTER TASK projectdb.table_schema_dim.load_dim_customer_gold RESUME;
ALTER TASK projectdb.table_schema_dim.process_customer_scd2 RESUME;
...
SHOW TASKS IN SCHEMA gold_schema;
